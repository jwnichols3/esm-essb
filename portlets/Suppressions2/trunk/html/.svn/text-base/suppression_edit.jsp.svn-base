...
<%
    ...
    List <SuppressionAudit> list = Suppression.retrievePreviousVersions ( Long.parseLong ( param_map.get ( "suppress_id" ) ) );
    Suppression suppressionRule = Suppression.select ( Long.parseLong ( strIndex ) );
    Iterator <SuppressionAudit> i = list.iterator();
    SuppressionAudit object = null;

    boolean show_col_row_id           = false;
    boolean show_col_suppress_id      = false;
    ...
    boolean show_col_notify_minutes   = true;
    boolean show_col_remove_on_reboot = true;
    boolean show_col_description      = false;
        ...
        <tr>
            <td>
                ...
                <html:form action="/edit_entry_process" method="post" onsubmit="submitForm(this); return false;">
                    <html:hidden name="suppression_edit_entry" property="suppressId" value='<%= strIndex %>' />
                    <table border="1">
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <html:text name="suppression_edit_entry" property="searchApps" onfocus="javascript:searchAppsFocus()" onblur="searchAppsOnblur()" onkeyup="searchAppsOnkeyup()" value="Type to search"/>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                &nbsp;
                                <html:text name="suppression_edit_entry" property="searchNodes" onfocus="javascript:searchNodesFocus()" onblur="searchNodesOnblur()" onkeyup="searchNodesOnkeyup()" value="Type to search"/>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                ...
                                <html:text name="suppression_edit_entry" property="searchDbServer" onfocus="javascript:searchDbServerFocus()" onblur="searchDbServerOnblur()" onkeyup="searchDbServerOnkeyup()" value="Type to search"/>
                            </td>
                        </tr>
                        <tr>
                            <td>&nbsp;</td>
                ...
                <table border="1">
                    <tr>
                       ...
                       <%
                        ...
                        %>
                       <%
                            if ( show_col_start_time && show_col_end_time )
                            {
                         %><th>
                    ...
                    <%
                      ...
                      %>
                    <tr>
                        <td><center><html:link action='<%= "/suppression_audit?method=audit&suppress_id=" + object.getSuppressId() + "&audit_version_num=" + object.getAuditVersionNum() %>'>View Audit</html:link></center></td>
                        ...
                        <%
                            if ( show_col_start_time && show_col_end_time )
                            {
                                 %><td>
                                        <center>
                                            <%= Toolkit.sdf.format ( object.getStartTime().getTime() ) %>
                                            <br>
                                            <%= Toolkit.sdf.format ( object.getEndTime().getTime() ) %>
                                        </center>
                                    </td><%
                            }
                         %>
                        <%
                            if ( show_col_ap_name )
                            {
                                 %><td><center><%= object.getAppName() %></center></td><%
                            }
                         %>
                        <%
                            if ( show_col_node_name )
                        ...
                        <%
                            ...
                            {
                                 %><td>
                                       <center><%= object.getDbServer() %></center>
                                       <hr>
                                       <%= object.getMessage() %>
                                </td><%
                            }
                        ...
                        <%
                            if ( show_col_notify_flg )
                            {
                                 %><td><center><%= object.getNotifyFlg() %></center></td><%
                            }
                         %>
                        <%
                            if ( show_col_notify_minutes )
...
<%
    ...
    {
        Calendar suppressStart = suppressionRule.getStartTime();
        Calendar suppressEnd   = suppressionRule.getEndTime();
        int start_hour         = suppressStart.get ( Calendar.HOUR );
        int end_hour           = suppressEnd.get   ( Calendar.HOUR );

        start_hour = ( start_hour != 0 )? start_hour : 12;
        end_hour   = ( end_hour   != 0 )? end_hour   : 12;

 %>
    document.suppression_edit_entry.supStartDate.value   = "<%= suppressStart.get ( Calendar.DATE )   %>";
    document.suppression_edit_entry.supStartMonth.value  = "<%= suppressStart.get ( Calendar.MONTH )  %>";
    document.suppression_edit_entry.supStartYear.value   = "<%= suppressStart.get ( Calendar.YEAR )   %>";
    document.suppression_edit_entry.supStartHour.value   = "<%= start_hour                            %>";
    document.suppression_edit_entry.supStartMinute.value = "<%= suppressStart.get ( Calendar.MINUTE ) %>";
    document.suppression_edit_entry.supStartAmpm.value   = "<%= (suppressStart.get(Calendar.HOUR_OF_DAY) >= 12)? "PM" : "AM" %>";

    document.suppression_edit_entry.supEndDate.value     = "<%= suppressEnd.get ( Calendar.DATE )   %>";
    document.suppression_edit_entry.supEndMonth.value    = "<%= suppressEnd.get ( Calendar.MONTH )  %>";
    document.suppression_edit_entry.supEndYear.value     = "<%= suppressEnd.get ( Calendar.YEAR )   %>";
    document.suppression_edit_entry.supEndHour.value     = "<%= end_hour                            %>";
    document.suppression_edit_entry.supEndMinute.value   = "<%= suppressEnd.get ( Calendar.MINUTE ) %>";
    document.suppression_edit_entry.supEndAmpm.value     = "<%= (suppressEnd.get(Calendar.HOUR_OF_DAY) >= 12)? "PM" : "AM" %>";
<%
    }
 %>
</script>
