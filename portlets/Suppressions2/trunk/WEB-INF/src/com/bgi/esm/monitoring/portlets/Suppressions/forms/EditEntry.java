...
public class EditEntry extends ActionForm implements SuppressionDate {
    ...
    public ActionErrors validate(
        ...
        {
            _log.info ( "Edit suppression validation for suppress ID #" + getSuppressId() + ":" );
            _log.info ( "\tStart:  " + sup_start_date.toString() );
            _log.info ( "\tEnd:    " + sup_end_date.toString() );
            _log.info ( "\tOffset: " + timezone_offset );
            _log.info ( "\tNow:   " + now.toString() );
            _log.info ( "\tStart(ms): " + date_start );
            _log.info ( "\tEnd(ms):   " + date_end );
            _log.info ( "\tNow(ms):   " + date_now );
        }
    ...
    public ActionErrors validate2(
        ...
        long date_end       = sup_end_date.getTime();
        long date_now       = now.getTime();
        System.out.println ( "\tStart(ms): " + date_start );
        System.out.println ( "\tEnd(ms):   " + date_end );
        System.out.println ( "\tNow(ms):   " + date_now );
