...
<%
    ...
    int suppress_id                  = 0;
    String struts_path               = "/search_my_history_again";
    ...
    ResultSet results                = Toolkit.retrieveHistory ( description, app_name, node, dbServer, dbServerMsg, startDate, endDate, status_reboot.intValue(), status_deleted.intValue(), username, sort_type );
 ...
 %>
    ...
    <table border="0" width="100%">
        ...
        <tr>
            <td>
                <b>Re-organizing Search Results for My Historical Entries</b>
				<br />
				<br />
                    ...
                    <table id="suppressionEntriesTable" border="1" width="100%">
                        <%
							while ( results.next() )
                            {
                                ...
                                {
                                    instance = results.getString ( "instance" );
                                    %><td nowrap><%= (null != instance)? instance: "&nbsp;" %></td><%
                                }
                                else if ( show_db_message )
                                {
                                    message  = results.getString ( "message" );
                                    %><td nowrap><%= (null != message)? message : "&nbsp;" %></td><%
                                }
                                if ( show_description )
                                {
                                    suppress_desc_txt = results.getString ( "suppress_desc_txt" );
                                    %><td nowrap><%= suppress_desc_txt %></td><%
                                }
                                ...
                                else if ( show_start_time )
                                {
                                    ...
                                    %><td><%= df.format ( start_utc_tms ) %></td><%
                                }
                                else if ( show_end_time )
                                {
                                    end_utc_tms   = results.getDate ( "end_utc_tms" );
                                    %><td><%= df.format ( end_utc_tms ) %></td><%
                                }
