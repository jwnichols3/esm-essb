...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.jpa.HttpCookie;
...
public class HttpCookieFinder implements IHttpCookieFinder
{
    ...
    {
        QuerySelectAll        = "SELECT x FROM HttpCookie x";
        QuerySelectCountAll   = "SELECT count(x) FROM HttpCookie x";
    }
    ...
    public IHttpCookie select ( String row_id )
    {
        ...
        try
        {
            ...
            IHttpCookie object = null;
                tx.begin();
                    object = (IHttpCookie) em.find ( HttpCookie.class, row_id );
                tx.commit();
            em.close();
    ...
    public boolean lockForUpdate ( IHttpCookie object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        {
            em = JPAFinderUtilities.getEntityManagerFactory().createEntityManager();
            IHttpCookie data = (IHttpCookie) em.getReference ( HttpCookie.class, object.getRowID() );
            data.setValue ( object );
            tx = em.getTransaction();
            tx.begin();
                em.merge ( data );
