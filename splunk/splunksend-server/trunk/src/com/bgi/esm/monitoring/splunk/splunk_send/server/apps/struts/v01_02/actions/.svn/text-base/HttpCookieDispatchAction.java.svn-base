...
import com.bgi.esm.monitoring.splunk.splunk_send.server.apps.struts.v01_02.forms.HttpCookieForm;
...
public class HttpCookieDispatchAction extends BaseDispatchAction
{
    final private static Logger _log = Logger.getLogger ( HttpCookieDispatchAction.class );
    ...
    {
        HttpCookieForm data_form = (HttpCookieForm) form;
        ...
        if ( false == isEnterpriseMode() )
        {
            ...
            IHttpCookie            object_select = finder.select ( PrimaryKey );
        ...
        else
        {
            try
            {
                IHttpCookieRemote bean = (IHttpCookieRemote) getContext().lookup ( "ejb/HttpCookieStateless" );
                IHttpCookie object_select = bean.selectRemote ( PrimaryKey );
                if ( null != object_select )
                {
                    data_form.setValue ( object_select );
                    result = true;
    ...
    public ActionForward add ( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
    {
        ...
        else
        {
            try
            {
                IHttpCookieRemote bean = (IHttpCookieRemote) getContext().lookup ( "ejb/HttpCookieStateless" );
                IHttpCookieFinder finder = Configuration.getInstance().getHttpCookieFinder();
                IHttpCookie object_insert = finder.newInstance();
    ...
    public ActionForward edit ( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
    {
        ...
        {
            IHttpCookieFinder finder = Configuration.getInstance().getHttpCookieFinder();
            IHttpCookie            object_select = finder.select ( data_form.getRowID() );
        ...
        else
        {
            ...
            {
                IHttpCookieRemote bean = (IHttpCookieRemote) getContext().lookup ( "ejb/HttpCookieStateless" );
                IHttpCookie object_select = bean.selectRemote ( data_form.getRowID() );
                if ( null != object_select )
                {
                    object_select.setValue ( data_form );
    ...
    public ActionForward browse ( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
    {
        ...
        List <IHttpCookie> browseRecords = (List <IHttpCookie>) session.getAttribute ( IHttpCookie.SESSION_BROWSE );
        int PageNum        = 0;
        int PageSize       = 0;
        ...
        if ( false == isEnterpriseMode() )
        {
            ...
            session.setAttribute ( IHttpCookie.SESSION_BROWSE, browseRecords );
    ...
    public ActionForward selectRecord ( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
    {
        ...
        if ( false == isEnterpriseMode() )
        {
            ...
            if ( null != object_select )
            {
                session.setAttribute ( IHttpCookie.SESSION_SELECT, object_select );
                result = true;
                if ( _log.isInfoEnabled() )
                {
                    WeakReference <StringBuilder> message = new WeakReference <StringBuilder> ( new StringBuilder() );
