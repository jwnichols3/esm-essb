...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.hibernate.HttpHeader;
...
public class HttpHeaderFinder implements IHttpHeaderFinder
{
    ...
    {
        QuerySelectAll        = "FROM HttpHeader x";
        QuerySelectCountAll   = "SELECT count(*) FROM HttpHeader";
    }
    ...
    public IHttpHeader select ( String row_id )
    {
        ...
        try
        {
            ...
            Session session = HibernateUtil.getCurrentSession();
            Transaction tx  = session.beginTransaction();
                object = (IHttpHeader) session.get ( HttpHeader.class, row_id );
            tx.commit();
    ...
    public boolean lockForUpdate ( IHttpHeader object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        try
        {
            ...
            IHttpHeader data = (IHttpHeader) session.get ( HttpHeader.class, object.getRowID(), LockMode.UPGRADE );
            data.setValue ( object );
            session = HibernateUtil.getCurrentSession();
            tx      = session.beginTransaction();
                session.update ( data );
