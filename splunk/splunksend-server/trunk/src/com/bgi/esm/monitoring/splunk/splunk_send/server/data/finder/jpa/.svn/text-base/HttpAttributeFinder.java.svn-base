...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.jpa.HttpAttribute;
...
public class HttpAttributeFinder implements IHttpAttributeFinder
{
    ...
    {
        QuerySelectAll        = "SELECT x FROM HttpAttribute x";
        QuerySelectCountAll   = "SELECT count(x) FROM HttpAttribute x";
    }
    ...
    public IHttpAttribute select ( String row_id )
    {
        ...
        try
        {
            ...
            IHttpAttribute object = null;
                tx.begin();
                    object = (IHttpAttribute) em.find ( HttpAttribute.class, row_id );
                tx.commit();
            em.close();
    ...
    public boolean lockForUpdate ( IHttpAttribute object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        {
            em = JPAFinderUtilities.getEntityManagerFactory().createEntityManager();
            IHttpAttribute data = (IHttpAttribute) em.getReference ( HttpAttribute.class, object.getRowID() );
            data.setValue ( object );
            tx = em.getTransaction();
            tx.begin();
                em.merge ( data );
