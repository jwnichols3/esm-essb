...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.hibernate.HttpAttribute;

public class HttpAttributeFinder implements IHttpAttributeFinder
{
    /**
     *
     */
    //  XXX___SERIALVER__XXX
    final private static Logger _log           = Logger.getLogger ( HttpAttributeFinder.class );
    protected String QuerySelectAll            = null;
    protected String QuerySelectCountAll       = null;

    public HttpAttributeFinder()
    {
        createFinderQueries();
    }
    ...
    {
        QuerySelectAll        = "FROM HttpAttribute x";
        QuerySelectCountAll   = "SELECT count(*) FROM HttpAttribute";
    }

    @SuppressWarnings ( "unchecked" )
    public List <IHttpAttribute> selectAll()
    {
        List <IHttpAttribute> list = null;
    ...
    public IHttpAttribute select ( String row_id )
    {
        ...
        try
        {
            IHttpAttribute object = null;
            Session session = HibernateUtil.getCurrentSession();
            Transaction tx  = session.beginTransaction();
                object = (IHttpAttribute) session.get ( HttpAttribute.class, row_id );
            tx.commit();
    ...
    public boolean update ( IHttpAttribute object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        try
        {
            ...
            IHttpAttribute data = select ( object.getRowID() );
            if ( null == data )
            {
                if ( _log.isEnabledFor ( Level.WARN ) )
                {
    ...
    public boolean lockForUpdate ( IHttpAttribute object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        try
        {
            ...
            IHttpAttribute data = (IHttpAttribute) session.get ( HttpAttribute.class, object.getRowID(), LockMode.UPGRADE );
            data.setValue ( object );
            session = HibernateUtil.getCurrentSession();
            tx      = session.beginTransaction();
                session.update ( data );
