...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.jpa.HttpParameter;
...
public class HttpParameterFinder implements IHttpParameterFinder
{
    ...
    {
        QuerySelectAll        = "SELECT x FROM HttpParameter x";
        QuerySelectCountAll   = "SELECT count(x) FROM HttpParameter x";
    }
    ...
    public IHttpParameter select ( String row_id )
    {
        ...
        try
        {
            ...
            IHttpParameter object = null;
                tx.begin();
                    object = (IHttpParameter) em.find ( HttpParameter.class, row_id );
                tx.commit();
            em.close();
    ...
    public boolean lockForUpdate ( IHttpParameter object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        {
            em = JPAFinderUtilities.getEntityManagerFactory().createEntityManager();
            IHttpParameter data = (IHttpParameter) em.getReference ( HttpParameter.class, object.getRowID() );
            data.setValue ( object );
            tx = em.getTransaction();
            tx.begin();
                em.merge ( data );
