...
import com.bgi.esm.monitoring.splunk.splunk_send.server.data.orm.hibernate.HttpParameter;

public class HttpParameterFinder implements IHttpParameterFinder
{
    /**
     *
     */
    //  XXX___SERIALVER__XXX
    final private static Logger _log           = Logger.getLogger ( HttpParameterFinder.class );
    protected String QuerySelectAll            = null;
    protected String QuerySelectCountAll       = null;

    public HttpParameterFinder()
    {
        createFinderQueries();
    }
    ...
    {
        QuerySelectAll        = "FROM HttpParameter x";
        QuerySelectCountAll   = "SELECT count(*) FROM HttpParameter";
    }

    @SuppressWarnings ( "unchecked" )
    public List <IHttpParameter> selectAll()
    {
        List <IHttpParameter> list = null;
    ...
    public IHttpParameter select ( String row_id )
    {
        ...
        try
        {
            IHttpParameter object = null;
            Session session = HibernateUtil.getCurrentSession();
            Transaction tx  = session.beginTransaction();
                object = (IHttpParameter) session.get ( HttpParameter.class, row_id );
            tx.commit();
    ...
    public boolean update ( IHttpParameter object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        try
        {
            ...
            IHttpParameter data = select ( object.getRowID() );
            if ( null == data )
            {
                if ( _log.isEnabledFor ( Level.WARN ) )
                {
    ...
    public boolean lockForUpdate ( IHttpParameter object, String username, String hostname, String remote_address, int remote_port )
    {
        ...
        try
        {
            ...
            IHttpParameter data = (IHttpParameter) session.get ( HttpParameter.class, object.getRowID(), LockMode.UPGRADE );
            data.setValue ( object );
            session = HibernateUtil.getCurrentSession();
            tx      = session.beginTransaction();
                session.update ( data );
