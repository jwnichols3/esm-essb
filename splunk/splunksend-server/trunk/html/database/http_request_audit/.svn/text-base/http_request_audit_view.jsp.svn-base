...
<table border="0" width="100%">
            ...
            <table width="100%" border="0">
                <tr>
                    ...
                    <%
                        if ( page_num > 0 )
                        {
                             %><html:link action='<%= String.format ( "/http_request_audit/view.do?page_size=%d&page_num=%d", page_size, page_num-1 ) %>'>Prev</html:link><%
                        }
                        else
                        {
                             %>&nbsp;<%
                    ...
                    <td><html:link action='<%= String.format ( "/http_request_audit/view.do?page_size=%d&page_num=%d", page_size, page_num+1 ) %>'>Next</html:link></td>
                </tr>
            </table>
        </td>
    </tr>
    <tr>
        <td>
            <html:link action="/http_request_audit/add">Add New Record</html:link>
            <table border="1">
                <tr>
                    <th>&nbsp;</th>
                    <th>&nbsp;</th>
                ...
                <%
                    ...
                    {
                        IHttpRequestAudit currentRecord = records.get ( counter );
                 %>
                <tr>
                    <td><html:link action='<%= "/http_request_audit/edit?PrimaryKey=" + currentRecord.getAuditRecordID() %>'>Edit</html:link></td>
                    <td><html:link action='<%= "/http_request_audit/select?PrimaryKey=" + currentRecord.getAuditRecordID() %>'>View</html:link></td>
