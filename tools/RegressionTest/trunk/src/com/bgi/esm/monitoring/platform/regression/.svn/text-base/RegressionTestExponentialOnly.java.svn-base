...
public class RegressionTestExponentialOnly extends Thread
{
    final private static Logger _log = Logger.getLogger ( RegressionTestExponentialOnly.class );
    ...
    final private static Vector <RegressionTestExponentialOnly> jobs  = new Vector <RegressionTestExponentialOnly> ();

    private int index = 0;
    public boolean isUser = true;
    private boolean is_finished = false;
    public RegressionTestExponentialOnly ( int index )
    {
        this.index = index;
    }
    ...
    public static void main ( String args[] )
    {
        ...
        if (( null != args ) && ( args.length > 1 ))
        {
            try
            {
                ...
                _log.info ( "Number of users specified: " + num_users );
            }
            catch ( NumberFormatException exception )
            {
                _log.error ( "Could not tell how many users were specified (" + args[1] + "), defaulting to 10" );

                num_users = 10;
            }
        }

        RegressionTestExponentialOnly eeb_thread = null;
        //  Start the EEB thread
        //eeb_thread = new RegressionTestExponentialOnly( 0 );
        ...
        RegressionTestExponentialOnly thread = null;
        for ( int c = 0; c < num_users; c++ )
        {
            for ( int counter = 1; counter <= num_records; counter++ )
            {
                try
                {
                    while ( jobs.size() >= num_users )
                    {
                        _log.info ( Integer.toString ( num_users ) + " active jobs detected at " + System.currentTimeMillis() );
                        Thread.sleep ( 500 );

                        scanForStoppedJobs();
                    }

                    thread = new RegressionTestExponentialOnly ( counter );
                    thread.start();
    ...
    private static void scanForStoppedJobs()
    {
        ...
        {
            RegressionTestExponentialOnly job = jobs.get ( j );
            if ( job.isFinished() )
            {
