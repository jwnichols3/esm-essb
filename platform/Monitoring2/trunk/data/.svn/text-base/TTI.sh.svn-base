...
# Author:    $Author: nichj $
# Edit date: $Date: 2005/09/27 00:19:43 $
#
# Description / Usage:
...
#
# 2005-05-23 nichj 2.00 Updating with logic around NetIQ interface.
#
#####################################################################
...
#
# For NetIQ AppManager messages, pass off to the perl script netiq-notify with all message parameters
# This will run the perl program that interfaces to Alarmpoint.  The message will then be passed on to
# Peregrine (as long as it is available).
#
if [ ${OPC_MSGGROUP} -eq "appmanager" ]; then
	echo "${OPC_MSGID} - Netiq Appmanager message, passing to netiq-notify. `date`" >> ${TTI_LOG_FILE}
