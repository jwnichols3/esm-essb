...
public class TestInvalidServiceCenterTicket extends TestCase {
    ...
    public TestInvalidServiceCenterTicket ( String param ) throws IOException, SAXException
    {
        ...
        assertNotNull ( "Could not retrieve schema from: " + SCHEMA_PATH, is );

		Source source = new StreamSource(is);
        assertNotNull ( "Could not create StreamSource", source );

		SchemaFactory sf = SchemaFactory.newInstance(W3C_XML_SCHEMA_NS_URI);
        assertNotNull ( "Could not create SchemaFactory", sf );

		ticketSchema = sf.newSchema(source);
        assertNotNull ( "Could not create ticketSchema", ticketSchema );
