 ...
 *   reentrant="false"
 *   view-type="local" 
 *   local-jndi-name="${jndi.base}/MonotonicEjbLocalHome"
 *   description="monotonic sequence bean"
 * 
 * @ejb.persistence table-name="monotonic"
 * 
 * @ejb.finder 
 *   signature="MonotonicLocal findSequence()"
 *   query="SELECT OBJECT(x) FROM MonotonicEjb AS x WHERE x.key = 1"
 *   unchecked="true"
 * 
 ...
 */
public abstract class MonotonicBean implements EntityBean {

	/**
	 * @ejb.create-method
	 */
	public MonotonicEjbPK ejbCreate(int key, int value) throws CreateException {
		setKey(key);
		setValue(value);
